module Reshume
    ...
    , -- Test-funcs
      test
    , test2 -- Types (expose to main)
    ) where

import Control.Applicative
import Data.Char
import Data.List
import Text.JSON (Result(Error, Ok), decode)
import Types
...
replaceKeys (content:rest) prefix vals =
    case left of
        v@(Just a) -> a
